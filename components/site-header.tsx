  ...
  return (
    <>
      <header className="sticky top-0 z-50 border-b border-[rgba(69,45,55,0.1)] bg-[rgba(255,252,248,0.92)] backdrop-blur-xl">
        ...
        <div className="container-luxe grid h-16 grid-cols-[1fr_auto] items-center gap-4 text-xs uppercase tracking-[0.2em] md:relative md:grid-cols-[auto_1fr_auto] md:h-[86px]">
          <button className="hidden md:inline-block" onClick={() => setMenuOpen(true)} aria-label="Open menu">
            Menu
          </button>
          <Link
            href="/"
            className="min-w-0 justify-self-start font-display text-[16px] font-semibold text-[var(--secondary)] md:absolute md:left-1/2 md:-translate-x-1/2 md:justify-self-auto md:text-[30px]"
          >
            <span className="block whitespace-nowrap tracking-[0.14em] sm:tracking-[0.18em] md:tracking-[0.2em]">
          ...
          <aside className="absolute left-0 top-0 h-full w-[92vw] max-w-[440px] overflow-y-auto border-r border-[rgba(69,45,55,0.1)] bg-[#fffaf6] p-5 shadow-[0_30px_80px_rgba(49,32,39,0.18)] md:p-7">
            <div className="mb-6 flex items-center justify-between border-b border-black/10 pb-4">
              <p className="font-display text-xl font-semibold text-black">
                <span className="block whitespace-nowrap tracking-[0.22em]">{siteConfig.shortName.toUpperCase()}</span>
              </p>
              <button
